...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _init() async {
    ...
    _updateWifiName();
    // 定时刷新 WiFi 名称
    Timer.periodic(const Duration(seconds: 30), (_) => _updateWifiName());
  ...
  Future<void> _updateWifiName() async {
    try {
      ...
      if (mounted) setState(() => _wifiName = name ?? '未连接');
    } catch (_) {
      if (mounted) setState(() => _wifiName = '未知');
    }
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text(
              '提示：仅个人自用。请将 App 加入电池优化白名单并锁定后台，防止被系统杀掉。',
              style: TextStyle(color: Colors.grey, fontSize: 12),
            ),
          ],
        ),
